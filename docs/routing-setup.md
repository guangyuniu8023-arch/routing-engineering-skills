...
从零搭建一个具备 skill 路由能力的 Agent 骨架。分两阶段执行：Phase A 收集需求并定义 skill 清单，Phase B 按 9 项架构约束生成目录和代码文件。产出物可直接进入 Layer 1-4 路由质量工程流程。
...
这些场景将在 Layer 1 中转化为 4-tuple 测试用例 `(case_id, asset_desc, user_text, expected_skill)`。
...
3. **成功标准**：
   ...
   - 整体：通过率 >= 目标阈值（Layer 1: 95%, Layer 4: 95% overall + 98% golden）
...
| `.routing-quality/config.md` | 供 Layer 1-4 使用的项目配置 |

完成后可直接进入 `/routing-layer1` 开始路由质量工程。
