...
AI 从所有已构建的 SKILL.md frontmatter 自动提取信息，生成 Baseline Intent 表格（兼容 phase0-discovery 配置模板）。每个 skill 一行，包含名称、意图、类型、allowed_actions。用户确认表格完整性即可。config.md 携带 Phase A 已确认的必须通过场景和不该走这里场景。Layer 1 在此基础上扩展（从 3-5 个扩到更多），而不是从零重建。
