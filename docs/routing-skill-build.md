...
本技能覆盖 Skill 工程的 **Phase C**（逐 Skill 构建循环）和 **Phase D**（闭合与衔接）。Phase C 完成每个 skill 从定义到可测的完整链路；Phase D 自动生成路由质量所需的配置和测试脚手架，并交接给 `/routing-layer1` 开始调优。
...
AI 从所有已构建的 SKILL.md frontmatter 自动提取信息，生成 Baseline Intent 表格（兼容 phase0-discovery 配置模板）。每个 skill 一行，包含名称、意图、类型、allowed_actions。用户确认表格完整性即可。其中"必须通过的场景"字段标记为 **"由 /routing-layer1 填充"**，留给后续路由质量调优阶段补全。
...
展示所有产出物状态，提示用户：**运行 `/routing-layer1` 开始路由质量调优**。

---

## 衔接协议

Phase C/D 的产出与路由质量层（layer1-4）之间通过 7 项衔接协议连接：
...
| Config | `.routing-quality/config.md` | phase0-discovery 读取 | Baseline Intent 表格 |
| Test cases | `routing_test_{N}skills.py` | layer1-4 测试 | 4-tuple |
...
6. **Config** — config.md 是 phase0-discovery 的入口，表格格式必须兼容后续层的自动解析。
7. **Test cases** — 初始 golden case 随 skill 构建同步产出，后续由 layer1-4 持续扩充。
...
|-----------|-----------|------|------------------|----------------|---------|----------|
| image-editing | 用户想对已有图片进行编辑修改 | generate:image | 由 /routing-layer1 填充 | — | — | — |
```
