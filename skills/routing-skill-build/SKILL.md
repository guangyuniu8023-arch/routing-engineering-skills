...
|-----------|-----------|------|------------------|----------------|---------|----------|
| image-generation | 用户想根据文字描述生成全新图片 | generate:image | 由 /routing-layer1 填充 | — | — | — |
```

"必须通过的场景"等字段标注 **"由 /routing-layer1 填充"**，留待路由质量调优阶段补全。

config.md 格式必须与 phase0-discovery 兼容。
...
4. config.md 格式必须与 phase0-discovery 兼容
...
| 6 | Config | phase0-discovery 读取 | Baseline Intent 表格格式一致 |
| 7 | Test cases | layer1-4 测试 | 4-tuple 格式贯穿全部层级 |
